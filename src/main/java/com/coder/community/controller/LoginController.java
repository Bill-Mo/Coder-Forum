...
public class LoginController implements CommunityConstant{
    ...
    public String register(Model model, User user) {
        ...
        if (map == null || map.isEmpty()) {
            model.addAttribute("msg", "Register success! We sent an activation email to you, please activate ASAP!");
    ...
    public void getKaptcha(HttpServletResponse response) {
        ...
        // Generate an ID for captcha
        String kaptchaOwner = CommunityUtil.generateUUID();
        Cookie cookie = new Cookie("kaptchaOwner", kaptchaOwner);
        cookie.setMaxAge(60);
        cookie.setPath(contextPath);
        response.addCookie(cookie);
        // Save kaptcha into Redis
        String redisKey = RedisKeyUtil.getKaptchaKey(kaptchaOwner);
        redisTemplate.opsForValue().set(redisKey, text, 60, TimeUnit.SECONDS);

        // Save image into session
    ...
    public String login(String username, String password, String code, boolean rememberMe, 
        ...
        if (kaptcha != null) {
            if (StringUtils.isBlank(kaptcha) || !kaptcha.equalsIgnoreCase(code)) {
                model.addAttribute("codeMsg", "Kaptcha code wrong!");
                return "/site/login";
            }
        } else {
            model.addAttribute("codeMsg", "Kaptcha expired!");
        }

        int expiredSec = rememberMe ? REMEMBERED_EXPIRED_SECONDS : DEFAULT_EXPIRED_SECONDS;
